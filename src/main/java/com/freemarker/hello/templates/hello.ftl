...
<body>
	...
	<br />
	<br />算数运算： <#-- 支持"+"、"－"、"*"、"/"、"%"运算符 --> <#assign number1=10
	number2=5 >
	<br />"+":${number1 + number2 }
	<br />"-":${number1 - number2 }
	<br />"*":${number1 * number2 }
	...
	<br />
	<br />比较运算符：
	<br /> <#if number1 + number2 gte 16> number1+number2 大于等于 16 <#elseif
	number1 + number2 == 15> number1+number2 等于 15 <#elseif number1 +
	number2 lt 15> number1+number2 小于 15 </#if>
	<br />
	<br />三元运算符：
	<br /> ${(users?size gt 15)?string('a','b') }
	<br />
	<br />内建函数： <#assign data = "abcd1234">
	<br />第一个字母大写：${data?cap_first}
	<br />所有字母小写：${data?lower_case}
	<br />所有字母大写：${data?upper_case} <#assign floatData = 12.34>
	...
	<table border="1" cellspacing="0" cellpadding="3">
		...
		<#if users?? && users?size gt 0> <#list users?sort_by("age")?reverse
		as user> <#if user??>

		<tr align='center'
			<#if user_index%2==0> style='background-color: #FF6347;' <#else>
			style='background-color: #FFD39B;' </#if>>
			<td>${user_index+1}</td>			
				<td>${user.id!"null"}</td>
				<td>${user.age!"null"}</td>
